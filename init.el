...
(use-package centuri
  ...
  :config
  (setq centuri--buffers-ignored '(" *MINIMAP*")))
...
(use-package vertico
  ...
  (defun my-vertico-select (index)
    (interactive)
    (vertico-next (if (string= vertico-preselect 'prompt) (+ index 1) index))
    ...
    (vertico-exit))
  (define-key minibuffer-local-map (kbd "C-0") (lambda () (interactive) (my-vertico-select 0)))
  (define-key minibuffer-local-map (kbd "C-1") (lambda () (interactive) (my-vertico-select 1)))
  (define-key minibuffer-local-map (kbd "C-2") (lambda () (interactive) (my-vertico-select 2)))
  (define-key minibuffer-local-map (kbd "C-3") (lambda () (interactive) (my-vertico-select 3)))
  (define-key minibuffer-local-map (kbd "C-4") (lambda () (interactive) (my-vertico-select 4)))
  (define-key minibuffer-local-map (kbd "C-5") (lambda () (interactive) (my-vertico-select 5)))
  (define-key minibuffer-local-map (kbd "C-6") (lambda () (interactive) (my-vertico-select 6)))
  (define-key minibuffer-local-map (kbd "C-7") (lambda () (interactive) (my-vertico-select 7)))
  (define-key minibuffer-local-map (kbd "C-8") (lambda () (interactive) (my-vertico-select 8)))
  (define-key minibuffer-local-map (kbd "C-9") (lambda () (interactive) (my-vertico-select 9)))
  )
...
(use-package ace-window
  ...
  :ensure t
  :config
  (defun custom-ace-window-fun (arg)
    (interactive "p")
    (ace-window arg)
    (pulsar-pulse-line-red))
...
(use-package treemacs
  ...
  (defun custom-treemacs-window-switch (&optional arg)
    (interactive "P")
  ...
  ;; :hook
  ;; (prog-mode . treemacs-mode-select-window)
  )
...
(use-package windresize
  :ensure t
  :config
  (global-set-key (kbd "C-=") 'windresize))
...
(use-package popwin
  ...
  :config
  (popwin-mode 1)
  (push '(vundo-mode :height 0.5 :position bottom) popwin:special-display-config)
  )
...
(use-package vundo
  ...
  (set-face-attribute 'vundo-default nil :family "monospace"))
...
(use-package centaur-tabs
  ...
  (defun centaur-tabs-buffer-groups ()
    ...
    (list
     (cond
      ((string-prefix-p "*shell" (buffer-name))
       "Shell")

      ((string-prefix-p "*dashboard" (buffer-name))
       "Dashboard")
...
(add-hook 'prog-mode-hook
          (lambda () (set-face-attribute 'default nil :weight 'light)))

(add-hook 'prog-mode-hook
          'custom-prog-mode-hook)

(add-hook 'prog-mode-hook
          'custom-prettify-mode-hook)
...
(custom-set-faces
 ...
 ;; Your init file should contain only one such instance.
 ;; If there is more than one, they won't work right.
 '(default ((t (:inherit nil :extend nil :stipple nil :background "white" :foreground "#37474F" :inverse-video nil :box nil :strike-through nil :overline nil :underline nil :slant normal :weight light :height 125 :width normal :foundry "GOOG" :family "RobotoMono Nerd Font"))))
 '(ahs-definition-face ((t (:background "light steel blue" :foreground "black" :weight normal))))
 '(ahs-definition-face-unfocused ((t (:background "light steel blue" :foreground "black" :weight normal))))
 '(ahs-face ((t (:background "gray90" :weight normal))))
 '(ahs-face-unfocused ((t (:background "gray90" :weight normal))))
 '(ahs-plugin-default-face ((t (:background "gray97" :foreground "black"))))
 '(ahs-plugin-default-face-unfocused ((t (:background "gray97" :foreground "black"))))
 '(aw-background-face ((t nil)))
 '(aw-leading-char-face ((t (:extend t :background "white" :foreground "red" :box (:line-width (2 . 2) :color "red" :style released-button) :weight extra-bold :height 3.0))))
 '(centaur-tabs-active-bar-face ((t (:background "red2" :foreground "white" :weight normal))))
 '(centaur-tabs-selected ((t (:background "red2" :foreground "white" :underline nil :weight semi-light))))
